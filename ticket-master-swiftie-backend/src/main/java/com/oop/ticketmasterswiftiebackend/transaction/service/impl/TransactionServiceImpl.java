...
public class TransactionServiceImpl implements TransactionService {
    ...
    public TransactionResponse createTransaction(CreateTransactionRequest createTransactionRequest) {
        ...
        if (createTransactionRequest.getStatus() != TransactionStatus.FAILED) {
            ticketEntity = ticketService.getTicketEntityById(createTransactionRequest.getTicketId());
            log.info("Ticket found: {}", ticketEntity);
        }
        LocalDateTime localDateTime = LocalDateTime.ofInstant(Instant.ofEpochMilli(Long.parseLong(
                String.valueOf(createTransactionRequest.getDateTime()))), ZoneId.systemDefault());
        TransactionEntity.TransactionEntityBuilder transactionBuilder = TransactionEntity.builder()
        ...
        if (ticketEntity != null) {
            transactionBuilder = transactionBuilder.ticket(ticketEntity)
                    .ticketId(ticketEntity.getTicketId());
        }
        TransactionEntity newTransaction = transactionBuilder.build();
        try {
    ...
    public List<TransactionEntity> getAllSuccessfulChargeTransactionByTicketIds(List<Integer> ticketIds) {
        return transactionRepository.findAllByTicketIdInAndStatusAndType(ticketIds, TransactionStatus.SUCCESS,
